...
All visual styling uses CSS custom properties (design tokens) defined in `crates/oxidgene-ui/src/components/layout.rs` (`LAYOUT_STYLES`). Dark theme is the default. Light theme is planned for EPIC D.
...
|---|---|---|
| `--bg-deep` | `#0d0f14` | Page background, deepest layer |
| `--bg-panel` | `#111318` | Panel / sidebar backgrounds, topbar |
| `--bg-card` | `#16191f` | Card backgrounds, input backgrounds |
| `--bg-card-hover` | `#1c2030` | Card hover state |
| `--border` | `#252d3d` | Borders, dividers, separators |
| `--border-glow` | `#e07820` | Focus/active border glow |
| `--sel-bg` | `#192038` | Selected item background |
...
| Token | Value | Purpose |
|---|---|---|
| `--text-primary` | `#ddd8cc` | Primary text, headings, names |
| `--text-secondary` | `#7a8da8` | Secondary text, labels, metadata |
| `--text-muted` | `#404f65` | Muted text, placeholders, disabled states |
...
| Token | Value | Usage |
|---|---|---|
| `--shadow-sm` | `0 1px 3px rgba(0,0,0,0.35)` | Cards, dropdowns |
| `--shadow-md` | `0 4px 16px rgba(0,0,0,0.55)` | Modals, popovers |
...
| Unknown | Grey | `var(--text-muted)` / `#404f65` |
...
## 10. Light Theme (Planned)

The light theme will override the same CSS custom properties. Planned token values:
...
| `--text-muted` | `#9e9488` |
| `--orange` | `#e07820` (unchanged) |
| `--orange-light` | `#f5a03a` (unchanged) |

The theme is toggled via a class on the root element (`<html class="light">`) and a toggle button in the [topbar](ui-topbar.md).

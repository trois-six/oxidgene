...
- **Alt-tabbing back to the desktop app no longer refreshes its current view.** The first fix protected only the pedigree's pan and zoom by ignoring a browser `resize` whose viewport dimensions had not changed. GTK also emits a native `configure` event whenever a window is remapped; Dioxus forwarded every one to `WebView::set_bounds`, which reallocates and repaints the whole WebView even when its geometry is identical. The desktop shell now discards a strictly duplicate `(position, size, scale factor)` before Dioxus creates the WebView, preserving the mounted state of the pedigree, person detail, media viewer and every other page. Real resizes, window moves and display/DPI changes still propagate. The filter covers both Wayland and X11; Windows and macOS retain their native event paths unchanged.
...
- [x] **Media tags.** A media or multi-page document owns an ordered list of free-form labels, shown as removable pastilles in the viewer. Every tag is a `media_tag` row: add/remove calls are atomic and case-insensitively idempotent, so a second editor cannot overwrite another person's tags. The same operations are exposed in REST and GraphQL. Tags are stored only on a document row, never copied onto its pages, so opening page 27 cannot show metadata that diverged from the other pages.
...
- [x] Preserve mounted page and viewer state across desktop focus changes; discard duplicate GTK geometry events without suppressing real resize or DPI adaptation.
- [ ] Thumbnail caching

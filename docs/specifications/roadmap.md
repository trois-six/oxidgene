...
- **Breaking:** `GqlPedigreeNode` exposes `birth` / `death` (`GqlProfileEvent`) in place of `birthYear` / `birthPlace` / `deathYear` / `deathPlace`, mirroring REST. And because `person_denorm` is a stored projection with no schema version, **existing rows must be rebuilt** — a re-import or `POST /trees/{id}/profiles/rebuild` — before any of this appears. Nothing detects that staleness yet; the same trap waits for the next projection field.

...
**Search semantics note:** FTS5 matches on **token prefixes** (`"perr"` matches `SAMPLE_SURNAME`, but `"sample_surname"` does not) instead of the previous arbitrary-substring matching; the PostgreSQL fallback keeps substring semantics. Prefix matching is the standard genealogy search UX and enables index-backed lookups.

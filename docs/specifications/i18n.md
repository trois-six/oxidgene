---
type: "Internationalization Specification"
title: "Internationalization (i18n) Specification"
description: "Localization scope, language switching behavior, and formatting conventions for OxidGene."
tags: [oxidgene, specification, i18n, localization]
...
- On first use (no stored preference): the app walks `navigator.languages` —
  the browser/OS list, ordered most-preferred first — and selects the first
  entry it has a translation for. Only the primary subtag is matched, so
  `fr-FR` and `fr_CA` both select French.
- English is used when no listed language is translated, and when detection
  fails entirely (blocked storage, missing `navigator.languages`)

### Desktop version

- Same mechanism as the web version: the WebView reports the OS locales
  through `navigator.languages`, and the choice persists in `localStorage`
...
A qualifier is rendered in **two registers**, and they are not interchangeable.

**Full text** — `format_date` in `components/date_input.rs`, used wherever a date
has a line to itself: the events panel, the person timeline, the identity
header, the edit modal. Translated through the `date_qualifier.*` keys.

| Qualifier | English | French |
|---|---|---|
| `Exact` | *(no prefix)* | *(no prefix)* |
| `About` | About | Environ |
| `Calculated` | Calculated | Calculée |
| `Estimated` | Estimated | Estimée |
| `Perhaps` | Perhaps | Peut-être |
| `Before` | Before | Avant |
| `After` | After | Après |
| `Or` | … or … | … ou … |
| `Between` | Between … and … | Entre … et … |
| `FromAge` | From an age | D'après un âge |

**Short marks** — `DateQualifier::short_prefix()` in `oxidgene-core`, for a
pedigree card, which has room for a year and nothing else. These are
**deliberately not translated**: they are GeneWeb's own symbols, which is what
Geneanet draws, so a user arriving from a Geneanet tree reads them already, and
`ca` is the same token in both our locales. See
[Tree View](ui-genealogy-tree.md) for the full table and the range handling.

Never spend the short marks where the full text fits — the marks exist to save
space, and a panel that has the room should say « vers 1620 », not « ca 1620 ».

### Gendered keys

French inflects past participles and Sex is data we hold, so « Né(e) le » is a
hedge worth spending only when the sex is genuinely unknown. Such keys come in
three: the bare key for `Sex::Unknown`, plus `_male` and `_female` suffixes.
The caller picks with a small helper rather than branching at each use site:

```
person.vitals.born_prefix          = "Né(e) le"      (Sex::Unknown)
person.vitals.born_prefix_male     = "Né le"
person.vitals.born_prefix_female   = "Née le"
```

English has no agreement, so its three keys carry the same string. That
redundancy is the point: the parity test requires both tables to hold the same
keys, and a language that *does* inflect gets the hooks without a code change.

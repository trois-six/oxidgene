...
async fn search_prefix_and_accent_folding() {
    ...
    let page = PersonSearchRepo::search(&db, tree_id, "perr", 10, 0)
        ...
        .unwrap();
    assert_eq!(page.total_count, 1);
    assert_eq!(page.entries[0].surname, "psample_surname");

    // Multi-word: all words must match (surname + given names).
    let page = PersonSearchRepo::search(&db, tree_id, "psample_surname pierre", 10, 0)
        .await
        .unwrap();
    assert_eq!(page.total_count, 1);

    // Word order doesn't matter.
    let page = PersonSearchRepo::search(&db, tree_id, "pierre psample_surname", 10, 0)
        .await
        .unwrap();
    assert_eq!(page.total_count, 1);

    // Non-matching word combination.
    let page = PersonSearchRepo::search(&db, tree_id, "psample_surname jean", 10, 0)
        .await
        .unwrap();
    assert_eq!(page.total_count, 0);
...
async fn search_performance_10k() {
    ...
    let surnames = [
        "Psample_surname", "Dupont", "Lefèvre", "Martin", "Bernard", "Moreau",
    ];
    ...
    let page = PersonSearchRepo::search(&db, tree_id, "psample_surname 17", 20, 0)
        .await
        .unwrap();

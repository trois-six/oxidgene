...
fn test_import_geneanet_names_parsed_from_value() {
    ...
    let pierre_name = result
        .person_names
        .iter()
        .find(|n| n.surname.as_deref() == Some("SAMPLE_SURNAME") && n.given_names.as_deref() == Some("Pierre"));
    assert!(pierre_name.is_some(), "Pierre SAMPLE_SURNAME not found");
}

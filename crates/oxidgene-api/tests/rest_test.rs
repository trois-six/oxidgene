...
async fn test_tree_crud() {
    ...
    let (status, body) = send_request(
        ...
        Some(serde_json::json!({
            "name": "Sample Family",
            "description": "The sample family tree"
        })),
    )
    .await;
    assert_eq!(status, StatusCode::CREATED);
    assert_eq!(body["name"], "Sample Family");
    assert_eq!(body["description"], "The sample family tree");
...
async fn test_person_name_crud() {
    ...
    let (status, body) = send_request(
        ...
        Some(serde_json::json!({
            "surname": "Psample_surname"
        })),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["surname"], "Psample_surname");

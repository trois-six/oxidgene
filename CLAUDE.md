...
**Import** (`docs/specifications/ui-import.md`, `ui-geneanet-import.md`): the tree card's `⋮` → Import opens `components/import_modal.rs` — a file tab (.ged/.gdz/.gw) and a five-step Geneanet tab. The extension picks the reader and each goes to its own endpoint; only **.gdz** (GEDZIP) carries media, ingested through the ordinary upload path so they land stored and thumbnailed (`import::import_gedzip` in `oxidgene-gedcom`, `service::gedcom::import_gedzip_and_persist`). Pipeline in `crates/oxidgene-geneanet`; server side in `api/src/service/geneanet.rs` + `rest/geneanet.rs`; login window in `apps/oxidgene-desktop/src/geneanet.rs`, reached through the `GeneanetCollector` trait so `oxidgene-ui` never sees `wry`. Steps 2 (archives), 3 (login) and the media half of 5 are **desktop-only**.
